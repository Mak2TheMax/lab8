...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    match intvl with
      | Empty -> false
      | Interval (low,high) -> if compare x low >= 0 && compare x high <= 0
    ...
    match intvl1, intvl2 with
     ...
     | Interval (x1, y1), Interval (x2, y2) ->
       if contains intvl1 x2 || contains intvl2 x1 then create
          (if compare x1 x2 < 0 then x1 else x2) (if compare y1 y2 > 0 then y1 else y2)
       else Empty
    end ;;
...
let intvl1 = IntInterval.create 0 3 ;;
let intvl2 = IntInterval.create 2 6 ;;
